...
/**
 * Handle OAuth2 callback from Google Calendar
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Handle errors from Google OAuth2
    if (error) {
    ...
    // Validate the code
    if (!code) {
      console.error('No authorization code provided for Google Calendar');
    ...
    return NextResponse.redirect(
      new URL(`/auth/complete?code=${code}&provider=Google&type=calendar`, request.url)
    );
  } catch (error) {

...
/**
 * Handle OAuth2 callback from Outlook Calendar
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Validate the code
    if (!code) {
      console.error('No authorization code provided for Outlook Calendar');
    ...
    return NextResponse.redirect(
      new URL(`/auth/complete?code=${code}&provider=Outlook&type=calendar`, request.url)
    );
  } catch (error) {
    console.error('Error handling Outlook Calendar OAuth callback:', error);
